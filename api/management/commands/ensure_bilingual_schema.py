...
from api.localization import translate_generated_text
        ...
        with transaction.atomic():
            for model in api_config.get_models():
                ...
                for english_name, english_field in model_fields.items():
                    ...
                    for obj in model.objects.only("pk", base_name, english_name).iterator(chunk_size=500):
                        current = getattr(obj, english_name, None)
                        source = getattr(obj, base_name, None)
                        if current not in (None, "") or not isinstance(source, str) or not source.strip():
                            continue
