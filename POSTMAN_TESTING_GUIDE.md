...
- رپ (Rap) - with sub-genres: رپ ایرانی, ترپ, هیپ هاپ, رپ underground
- الکترونیک (Electronic) - with sub-genres: هاوس, ترنس, تکنو, EDM
- جز (Jazz) - with sub-genres: جز کلاسیک, بیباپ, جز مدرن, جز فیوژن
- بلوز (Blues) - with sub-genres: بلوز کلاسیک, دلتا بلوز, الکتریک بلوز, بلوز راک
- متال (Metal) - with sub-genres: هوی متال, بلک متال, دث متال, متال‌کور
- کلاسیک (Classical) - with sub-genres: اپرا, سمفونی, کنسرتو, سونات
- فولک (Folk) - with sub-genres: فولک ایرانی, فولک آمریکایی, فولک اروپایی, فولک مدرن

**Moods (15 moods):**
شاد, غمگین, عاشقانه, انرژیک, آرام, هیجان‌انگیز, مذهبی, پارتی, تمرکز, خواب, ورزشی, موتورسواری, نوستالژیک, الهام‌بخش, رقص

**Tags (20 tags):**
تابستانی, زمستانی, بهاری, پاییزی, جدید, کلاسیک, ویژه, پرفروش, جدیدترین‌ها, محبوب, وایرال, تیتریاژ, تبلیغاتی, فستیوال, کنسرت, ریمیکس, اصل, کاور, لایو, استودیویی
...
1. **Run Initial Setup** → Execute `python manage.py create_initial_data` on server
2. **Register/Login** → Get access token
3. **Create Artist** → Note the artist ID (e.g., 1)
4. **Use Existing Genres/SubGenres/Moods/Tags** → Check available IDs via GET endpoints
5. **Create Album** (optional) → Note album ID
6. **Upload Song** → Use form-data with:
   - audio_file: your.mp3
   - title: "Test Song"
   - audio_file: your.mp3
   - title: "Test Song"
