...
# Generated migration files intentionally remain in the working tree. The
# image build below copies them into /app, and the post-build migrate step then
# applies them to the database. Deleting one here recreates it every deployment.
set +e
if command -v python >/dev/null 2>&1; then
  echo "Running: python manage.py makemigrations --noinput --skip-checks"
  ...
  if command -v docker >/dev/null 2>&1; then
    ...
    echo "No docker available to run makemigrations." >&2
    MSTATUS=1
  fi
fi
if [ "$MSTATUS" -ne 0 ]; then
  echo "makemigrations failed with status $MSTATUS; aborting before stopping services or building the image." >&2
  exit "$MSTATUS"
fi
...
echo "Repairing initial genre and sub-genre records (one-time)"
$COMPOSE_CMD run --rm --no-deps "$WEB_SERVICE" python manage.py repair_initial_genres

echo "Backfilling playlist unique IDs"
$COMPOSE_CMD run --rm --no-deps "$WEB_SERVICE" python manage.py ensure_playlist_unique_ids
